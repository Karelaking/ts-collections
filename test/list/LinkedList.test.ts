...
describe("LinkedList - Runtime Type Safety", () => {
	...
	it("should allow mixed types when strict is false", () => {
		const list = new LinkedList<string | number | boolean>({ strict: false });
		list.add(1 as unknown as string | number | boolean);
		list.add("text" as unknown as string | number | boolean);
		list.add(true as unknown as string | number | boolean);
		expect(list.size()).toBe(3);
	});
