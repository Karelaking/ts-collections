...
export function describeStack(createStack: () => Stack<number>): void {
  describe("Stack Interface", () => {
    ...
    describe("search", () => {
      it("should return 1-based distance from top", () => {
        stack.push(10);
        stack.push(20);
        stack.push(30);

        // Top is 30 -> distance 1
        expect(stack.search(30)).toBe(1);
        // Next is 20 -> distance 2
        expect(stack.search(20)).toBe(2);
        // Bottom is 10 -> distance 3
        expect(stack.search(10)).toBe(3);
      });

      it("should return -1 if element is not found", () => {
        stack.push(10);
        expect(stack.search(99)).toBe(-1);
      });

      it("should return -1 on an empty stack", () => {
        expect(stack.search(10)).toBe(-1);
      });
    });
